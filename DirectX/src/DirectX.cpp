...
void Render()
{
    ...
    // Present
    {
        ...
        g_FrameFenceValues[m_swapChain->getCurrentBackBufferIndex()] = m_commandQueue->signal();
        //g_FrameFenceValues[m_swapChain->getCurrentBackBufferIndex()] = Signal(g_CommandQueue, g_Fence, g_FenceValue);

        //g_CurrentBackBufferIndex = g_SwapChain->GetCurrentBackBufferIndex();

        m_commandQueue->waitForFenceValue(g_FrameFenceValues[m_swapChain->getCurrentBackBufferIndex()]);
...
int CALLBACK wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR lpCmdLine, int nCmdShow)
{
    ...
    m_commandQueue = std::make_shared<CommandQueue>(g_Device, D3D12_COMMAND_LIST_TYPE_DIRECT);

    //g_CommandQueue = CreateCommandQueue(g_Device, D3D12_COMMAND_LIST_TYPE_DIRECT);


    m_swapChain = std::make_shared<SwapChain>(g_Device, m_commandQueue->getCommandQueue(), settings.width, settings.height,
